<?php 
 ...
 if (!isset($_SESSION['email'])) {
        ...
        $members = $conn->query("SELECT * FROM `exercise3` WHERE `email_address`='$email'");
        ...
        if(isset($_POST['submit']))
        {
            ...
            $check_email = $conn->query("SELECT email_address FROM exercise3 WHERE email_address='$email'");
            ...
            if($answer == 'B') {
               $query = "INSERT INTO exercise3 (id,email_address,answer,created_at) VALUES('','$email','$answer','$timestamp')";
              mysqli_query($conn,$query) or die(mysqli_error($conn));
            
              $msgreg = "<div class='alert alert-success' style='font-size:20px;'>Correct!!! Please click the <b>Next ></b> button at the bottom-right to proceed with the various types of phishing attacks.</div> ";
                $conn->close(); 
            }else{
               $query = "INSERT INTO exercise3 (id,email_address,answer,created_at) VALUES('','$email','$answer','$timestamp')";
              mysqli_query($conn,$query) or die(mysqli_error($conn));
            
              $msgreg = "<div class='alert alert-danger' style='font-size:20px;'>Wrong... Please click the <b>Next ></b> button at the bottom-right to proceed with the various types of phishing attacks.</div>";
                $conn->close();
            }
          }else{
...
<html lang="en">
  <head>
  	<title>Excercise 3 | eLearning</title>
    <meta charset="utf-8">
  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" style="background-color: #f5f5f5; position: relative;">
        ...
        <div class="prevnext">
             ...
             <div class="">
                <a href="04-examples-of-phishing-attacks.php">
