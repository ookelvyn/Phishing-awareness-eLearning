<?php 
 ...
 if (!isset($_SESSION['email'])) {
        ...
        $members = $conn->query("SELECT * FROM `exercise4` WHERE `email_address`='$email'");
        ...
        if(isset($_POST['submit']))
        {
            ...
            $check_email = $conn->query("SELECT email_address FROM exercise4 WHERE email_address='$email'");
            ...
            if($answer == 'E') {
               $query = "INSERT INTO exercise4 (id,email_address,answer,created_at) VALUES('','$email','$answer','$timestamp')";
              mysqli_query($conn,$query) or die(mysqli_error($conn));
            
              $msgreg = "<div class='alert alert-success' style='font-size:20px;'>Correct!!! Phishing attacks can target anyone who uses technology and holds certain values, regardless of whether they are an individual, entity or group. Please click the <b>Next ></b> button at the bottom-right to proceed.</div> ";
                $conn->close(); 
            }else{
               $query = "INSERT INTO exercise4 (id,email_address,answer,created_at) VALUES('','$email','$answer','$timestamp')";
              mysqli_query($conn,$query) or die(mysqli_error($conn));
            
              $msgreg = "<div class='alert alert-danger' style='font-size:20px;'>Wrong answer... Phishing attacks can target anyone who uses technology and holds certain values, regardless of whether they are an individual, entity or group. Please click the <b>Next ></b> button at the bottom-right to proceed.</div>";
                $conn->close();
            }
          }else{
...
<html lang="en">
  <head>
  	<title>Excercise 4 | eLearning</title>
    <meta charset="utf-8">
      ...
      <div id="content" class="p-4 p-md-5 pt-5" style="background-color: #f5f5f5; position: relative;">
        ...
        <div class="content-holder" >
          ...
          <div class="answers">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" name="option" id="flexRadioDefault5" value="E" required>
