<?php 
 ...
 if (!isset($_SESSION['email'])) {
        ...
        $members = $conn->query("SELECT * FROM `exercise6` WHERE `email_address`='$email'");
        ...
        if(isset($_POST['submit']))
        {
            ...
            $check_email = $conn->query("SELECT email_address FROM exercise6 WHERE email_address='$email'");
            ...
            if($answer == 'C') {
               $query = "INSERT INTO exercise6 (id,email_address,answer,created_at) VALUES('','$email','$answer','$timestamp')";
              mysqli_query($conn,$query) or die(mysqli_error($conn));
            
              $msgreg = "<div class='alert alert-success' style='font-size:20px;'>Correct!!! Please click the <b>Next ></b> button at the bottom-right to proceed with Principle of persuasion and the various types</div> ";
                $conn->close(); 
            }else{
               $query = "INSERT INTO exercise6 (id,email_address,answer,created_at) VALUES('','$email','$answer','$timestamp')";
              mysqli_query($conn,$query) or die(mysqli_error($conn));
            
              $msgreg = "<div class='alert alert-danger' style='font-size:20px;'>Wrong answer... Please click the <b>Next ></b> button at the bottom-right to proceed with Principle of persuasion and the various types.</div>";
                $conn->close();
            }
          }else{
...
<html lang="en">
  <head>
  	<title>Excercise 6 | eLearning</title>
    <meta charset="utf-8">
  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" style="background-color: #f5f5f5; position: relative;">
        ...
        <div class="content-holder" >
          ...
          <div class="questions">
            <h4>
              What is the persuasion principle that describes being influenced by someone due to sharing the same tribal affiliation? 
